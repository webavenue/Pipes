...
namespace UTNotifications.WSA
{
    ...
    //public
        ...
        {
            PostLocalNotification(null, title, text, id, userData, notificationProfile);
        }

        private static void PostLocalNotification(string provider, string title, string text, int id, IDictionary<string, string> userData, string notificationProfile)
        {
            ScheduleNotification(provider, ImmediateInSeconds, title, text, id, userData, notificationProfile);
        }
        ...
        {
            ScheduleNotification(null, triggerInSeconds, title, text, id, userData, notificationProfile);
        }

        private static void ScheduleNotification(string provider, int triggerInSeconds, string title, string text, int id, IDictionary<string, string> userData, string notificationProfile)
        {
            if (triggerInSeconds <= 0 && triggerInSeconds != ImmediateInSeconds)
            {
            ...
            RegisterNotification(provider, triggerInSeconds, 0, title, text, id, userData, notificationProfile, 0);
        }
        ...
        public static void ScheduleNotificationRepeating(int firstTriggerInSeconds, int intervalSeconds, string title, string text, int id, IDictionary<string, string> userData, string notificationProfile)
        {
            ...
            RegisterNotification(null, triggerInSeconds, intervalSeconds, title, text, id, userData, notificationProfile, GetCurrentUnixTimeSeconds() + firstTriggerInSeconds - intervalSeconds - 1);
        }
        ...
        public static void HandleReceivedNotifications(string appArguments, out IList<Notification> allNotifications, out Notification clickedNotification)
        {
            ...
            if (HandleReceivedNotifications() && NotificationsEnabled())
            {
                    ...
                    if (it.StartsWith(STORE_KEY_PREFIX))
                    {
                        try
                        {
                            ...
                            if (handle)
                            {
                                ...
                                allNotifications.Add(ReadNotification(registeredNotification));
        ...
        public static void PushNotificationReceived(string content)
        {
            ...
            if (id < 0)
            {
                id = GetNextPushNotificationId();
            }

            string notificationProfile = ExtractStringValue(json, notificationProfileFieldName);

            PostLocalNotification("WNS", title, text, id, GetUserData(json, userDataFieldName), notificationProfile);
        }

        public static bool NotificationRegistered(int id)
        {
            return ApplicationData.Current.LocalSettings.Values.ContainsKey(STORE_KEY_PREFIX + id);
        }
    ...
    //private
        ...
        private static void RegisterNotification(string provider, int lastTriggerInSecondsFromNow, int intervalSeconds, string title, string text, int id, IDictionary<string, string> userData, string notificationProfile, long lastTimeHandled)
        {
            ApplicationDataCompositeValue registeredNotification = new ApplicationDataCompositeValue();
            if (intervalSeconds == 0 && lastTriggerInSecondsFromNow == ImmediateInSeconds)
            {
                registeredNotification["immediate"] = true;
                lastTriggerInSecondsFromNow = 1;
            }
            if (!string.IsNullOrEmpty(provider))
            {
                registeredNotification["provider"] = provider;
            }
        ...
        private static void Reschedule(ApplicationDataCompositeValue registeredNotification, bool onEnabled)
        {
            ...
            string provider = registeredNotification.ContainsKey("provider") ? (string)registeredNotification["provider"] : null;
            int lastTriggerInSecondsFromNow = (int)((long)registeredNotification["lastTriggerInSeconds"] - currentTime);
            int scheduleInAdvanceSeconds = Math.Min(ScheduleRepeatedForSeconds + intervalSeconds, MaxRepeatsSchedulingAtOnce * intervalSeconds);
            ...
            if (intervalSeconds > 0)
            {
                ...
                if (lastTriggerInSecondsFromNow + intervalSeconds < scheduleInAdvanceSeconds)
                {
                    ...
                    RegisterNotification(provider, triggerInSeconds, intervalSeconds, title, text, id, userData, notificationProfile, lastTimeHandled);
                }
            }
            else
            {
        ...
        {
            if (triggerInSeconds == ImmediateInSeconds)
            {
                triggerInSeconds = 1;
            }
        ...
        {
            return (long)(DateTime.UtcNow - UNIX_BASE_DATE_TIME_UTC).TotalSeconds;
        }
        ...
        private static bool HandleReceivedNotifications()
        {
            var settings = ApplicationData.Current.LocalSettings;
            return settings.Values.ContainsKey(HANDLE_NOTIFICATIONS_KEY) ? (bool)settings.Values[HANDLE_NOTIFICATIONS_KEY] : false;
        }

        private static Notification GetClickedNotification(string appArguments, ApplicationDataContainer settings)
        {
            ...
            {
                if (appArguments[i] != '-' && !char.IsDigit(appArguments[i]))
                {
                    break;
                }
                else
            ...
            string key = STORE_KEY_PREFIX + argValue;
            if (!settings.Values.ContainsKey(key))
            {
                return null;
            }

            return ReadNotification((ApplicationDataCompositeValue)settings.Values[key]);
        }

        private static Notification ReadNotification(ApplicationDataCompositeValue registeredNotification)
        {
            if (registeredNotification != null)
            {
                long triggerUnixTime;
                if (!registeredNotification.ContainsKey("immediate"))
                {
                    triggerUnixTime = (long)registeredNotification["lastTriggerInSeconds"];
                }
                else
                {
                    triggerUnixTime = 0;
                }
                return new Notification(triggerUnixTime, (int)registeredNotification["intervalSeconds"], registeredNotification.ContainsKey("provider") ? (string)registeredNotification["provider"] : null, (string)registeredNotification["title"], (string)registeredNotification["text"], (int)registeredNotification["id"], UnpackUserData(registeredNotification), (string)registeredNotification["notificationProfile"], 0);
            }
            else
            {
                return null;
        ...
        private const int ImmediateInSeconds = -1;
        ...
        private static readonly DateTime UNIX_BASE_DATE_TIME_UTC = DateTime.SpecifyKind(new DateTime(1970, 1, 1), DateTimeKind.Utc);
